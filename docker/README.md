...
- **NEW: Interactive Configuration Mode** - Answer prompts instead of editing file
- Usage:
  ```bash
  # Method 1: Interactive mode (⭐ RECOMMENDED)
  ./docker/scripts/setup-instance.sh --interactive

  # Method 2: Edit configuration in script
...
- Interactive menu with 7 options:
  1. **🆕 Complete fresh deployment** - Runs entire workflow (setup → pull → build → deploy → schema)
...
- **Option 1 (Complete fresh deployment)** automatically:
  - **Step 1/5**: Runs setup-instance.sh (generate secrets, create .env)
  - **Step 2/5**: Pull Docker images (⭐ NEW - faster deployment)
  - **Step 3/5**: Builds Docker images
  - **Step 4/5**: Starts all services
  - **Step 5/5**: Waits for database and applies schema automatically
...
1. **Configure instance (choose one method):**

   **Method A: Interactive Mode (⭐ RECOMMENDED)**
   ```bash
   # Let the script ask you questions - no editing needed!
   ./docker/scripts/setup-instance.sh --interactive
   ```
...
2. **Run deployment script:**
   ...
   - ✅ **Step 4/5**: Start all services and wait for database
   - ✅ **Step 5/5**: Apply database schema automatically
...
- Access Studio via Kong URL (e.g., `https://studio.example.com`) - requires authentication
