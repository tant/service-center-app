...
create policy "profiles_insert_policy" on "profiles"
  for insert with check (auth.uid() = user_id or 'admin' = any((select roles from profiles where user_id = auth.uid())));
...
create policy "profiles_delete_policy" on "profiles"
  for delete using ('admin' = any((select roles from profiles where user_id = auth.uid())));