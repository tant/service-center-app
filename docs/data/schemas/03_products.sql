...
create table "products" (
  ...
  "brand" text,
  "model" text,
  "type" text check ("type" in ('hardware', 'software', 'accessory')),
  "primary_image" text,
...
create policy "products_delete_policy" on "products"
  for delete using (
    'admin' = any((select roles from profiles where user_id = auth.uid())) or
    'manager' = any((select roles from profiles where user_id = auth.uid()))
  );