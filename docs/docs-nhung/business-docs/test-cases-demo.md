...
|---|------|-------|--------|--------|
| 1 | UI | Remove "Phiếu nhập kho / Phiếu điều chỉnh (kiểm kê)" | Medium | **DONE** |
| 2 | UI | Hide Workflows module | Medium | **DONE** |
| 3 | UI/Feature | **Hide tất cả features liên quan đến Linh kiện (Parts):**<br>✅ **Đã ẩn:**<br>- Menu "Danh mục linh kiện" (Parts Catalog)<br>- Kho ảo "Kho Linh kiện" (`parts`) trong danh sách kho<br>- Trường/cột "Linh kiện" trong các form và bảng<br>- Options chọn kho linh kiện trong dropdown<br>- Page `/catalog/parts` redirect về products<br>- Parts selection trong product form<br>✅ **Implementation:**<br>- Không xóa dữ liệu, chỉ ẩn UI/UX<br>- Backend vẫn giữ logic để có thể enable lại sau<br>- Code gốc được comment với Issue #9 tag | Medium | **DONE** |
| 4 | UI | Remove "Phiếu xuất điều chỉnh (kiểm kê)" | Medium | **DONE** |
| 6 | UI | **Trang Tổng quan Kho hàng:**<br>✅ **Đã thực hiện:**<br>- Removed InventoryStatCards (warning cards) từ overview page<br>- Simplified inventory tables:<br>&nbsp;&nbsp;• All warehouses: 7→4 columns (Sản phẩm, SKU, Tồn kho, Xem chi tiết)<br>&nbsp;&nbsp;• Physical warehouses: 8→5 columns (+ Kho ảo)<br>&nbsp;&nbsp;• Virtual warehouses: 8→5 columns (+ Vị trí vật lý)<br>- Removed status filter dropdown và StockStatusBadge<br>- Renamed "Thực tế" → "Tồn kho" cho rõ ràng<br>✅ **Bonus - Stock Detail Page:**<br>- Simplified header: 4→1 metrics (chỉ "Tổng Tồn Kho")<br>- Simplified breakdown: removed "Khai báo" và "Chênh lệch"<br>- Virtual warehouse table: 4→2 columns | Medium | **DONE** |
...
| 21 | Bug | **Lỗi "URI too long" khi nhập số lượng lớn serial:**<br>🐛 **Hiện tượng:**<br>- Nhập ~500 serials vào phiếu → báo lỗi "URI too long"<br>📍 **Ảnh hưởng:**<br>- Phiếu nhập kho (Stock Receipt)<br>- Phiếu xuất kho (Stock Issue)<br>- Phiếu chuyển kho (Transfer)<br>- Lô RMA (RMA Batch)<br>🔍 **Root Cause:**<br>- tRPC's httpBatchLink gửi data qua URL query params<br>- ~500 serials @ 12 chars = ~8KB URL → vượt giới hạn server<br>✅ **Solution Implemented:**<br>- Client-side chunking: Chia requests thành chunks 50 serials/lần<br>- Progress indicator: Hiển thị "Đang lưu... (7/10)"<br>- Backend validation: Max 100 serials/request làm safeguard<br>- Files modified: unified-serial-input-drawer.tsx, add-products-to-rma-drawer.tsx, receipts.ts, issues.ts, transfers.ts, physical-products.ts<br>- Tested với 500+ serials: ✅ Success | High | **DONE** |
...
| 28 | TC-INV-001 (Test 1) | Bước 1 | UX | Trường ngày tháng: cho phép nhập tự do theo format dd/mm/yy, tự động thêm dấu `/` phân cách và hiển thị lịch theo best practice | Medium | **DONE** |
