...
*   **Backend (API):**
    *   Cập nhật adapter `ServiceTicketAdapter.onTaskComplete` để chuyển phiếu từ `in_progress` sang `ready_for_pickup` khi tất cả task bắt buộc hoàn tất (thay vì chuyển thẳng sang `completed`).
