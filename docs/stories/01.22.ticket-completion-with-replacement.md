...
- Update ticket status to `ready_for_pickup`, set outcome, replacement_product_id
- Create 2 stock transfers (auto-approved):
  - **Outbound**: Replacement product `warranty_stock` → `customer_installed`
  - **Inbound**: Old product `customer_installed` → `in_service`
- Set `last_known_customer_id` and `status = 'issued'` on replacement product
- Triggers auto-generate stock_issue + stock_receipt for each transfer
...
**AC8**: Create "Sản phẩm đổi trả" Card on ticket detail page:
...
**AC9**: Create "Hoàn thành phiếu" Modal:
...
- Automatic inbound transfer when creating ticket (Luồng 1 in AUTO-TRANSFER doc)

## Implemented Beyond Original Scope

The following items were originally out of scope but have been implemented:
- ✅ **Handling of old product**: Old product is moved to `in_service` (Kho Đang Sửa Chữa) via stock transfer
- ✅ **Auto-generating stock_transfer documents**: System creates proper stock_transfer records instead of direct warehouse update
...
### Happy Path
1. Complete ticket with `outcome = 'repaired'` → ticket completed, no inventory change
2. Complete ticket with `outcome = 'warranty_replacement'` + valid product → ticket completed, inventory updated
3. Complete ticket with `outcome = 'unrepairable'` → ticket completed, no inventory change

### Validation Errors
4. Missing `replacement_product_id` when `outcome = 'warranty_replacement'` → error
5. Replacement product not in `warranty_stock` → error
6. Replacement product different `product_id` → error
7. Complete ticket from invalid status (`pending`, `cancelled`) → error
