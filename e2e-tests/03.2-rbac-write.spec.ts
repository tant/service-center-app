...
test.describe("RBAC Write Operations", () => {
  test.describe("Team Management Operations", () => {
    test("Admin can change user role", async ({ page }) => {
      ...
      const techRow = page.getByTestId(
        `team-member-row-${testUsers.technician.email}`,
      );
      await expect(techRow).toBeVisible({ timeout: 5000 });
      const roleButton = techRow.getByRole("button", {
      ...
      const receptionOption = page.locator(
        '[role="menuitem"][data-role="reception"]',
      );
      await receptionOption.click();
      await expect(page.getByText(/vai trò đã được cập nhật/i)).toBeVisible({
        timeout: 5000,
      });
      // Revert change to keep state clean
      await roleButton.click();
      await page.waitForTimeout(500);
      const technicianOption = page.locator(
        '[role="menuitem"][data-role="technician"]',
      );
    ...
    test("Admin can reset password for a user", async ({ page }) => {
      ...
      await page.goto("/team");
      const tempUser = {
        email: `pw-reset-${Date.now()}@example.com`,
      ...
      await page
        .getByRole("option", { name: tempUser.role, exact: true })
        .click();
      ...
      await expect(page.getByText(/đặt lại mật khẩu thành công/i)).toBeVisible({
        timeout: 5000,
      });
      await logout(page);
    });
    ...
    test("Admin can activate/deactivate user", async ({ page }) => {
      ...
      await expect(
        page.getByText(/tài khoản đã được vô hiệu hóa/i),
