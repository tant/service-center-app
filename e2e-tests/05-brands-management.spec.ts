...
test.describe("Brands Management", () => {
  ...
  test("should allow creating a new unique brand", async ({ page }) => {
    ...
    await expect(
      page.getByRole("cell", { name: uniqueBrandName }),
    ).toBeVisible();
  });

  test("should show validation error if brand name is empty", async ({
    page,
  }) => {
    await page.getByTestId("add-brand-button").click();
  ...
  test("should allow searching for a brand", async ({ page }) => {
    await page
      .getByPlaceholder("Tìm theo tên thương hiệu...")
      .fill(searchBrand);

    const tableRows = page.locator("tbody tr");
    await expect(tableRows.first()).toBeVisible();
    await expect(
      tableRows.first().getByRole("cell", { name: searchBrand }),
    ).toBeVisible();
  });

  test("should allow editing an existing brand and reverting the change", async ({
    page,
  }) => {
    // --- Edit Step ---
    ...
    await expect(
      page.getByText("Thương hiệu đã được cập nhật thành công"),
    ).toBeVisible();
    await expect(
      page.getByRole("cell", { name: updatedEditBrandName }),
    ).toBeVisible();

    // --- Revert Step (to ensure test idempotency) ---
    const updatedBrandRow = page.getByRole("row", {
      name: updatedEditBrandName,
    });
