import { test as setup, expect, FullConfig, chromium } from '@playwright/test';
import { adminUser, testUsers } from './test-data';
import { login, searchUser, isUserActive, activateUser } from './helpers';

async function globalSetup(config: FullConfig) {
  console.log('--- Starting Global Setup: Ensuring baseline data exists via UI ---');
  ...
  try {
    ...
    const mainContentVisible = await page.getByRole("main").isVisible({ timeout: 5000 }).catch(() => false);

    if (!mainContentVisible) {
      console.log('[GlobalSetup] Admin login failed or profile missing, attempting initial setup...');
      ...
      console.log('[GlobalSetup] ✓ Admin user already exists and is logged in.');
    }
    ...
    for (const role of Object.keys(testUsers) as Array<keyof typeof testUsers>) {
      ...
      if (userFound) {
        ...
        if (!isActive) {
          console.log(`[GlobalSetup] ⚠ User ${user.email} is deactivated. Activating...`);
          await activateUser(page, user.email);
        }
        continue;
      }

      console.log(`[GlobalSetup] Creating new user: ${user.email} with role: ${user.role}`);
    ...
    for (const brandName of requiredBrands) {
      ...
      if (await noResultsText.isVisible() || !(await brandCell.isVisible())) {
        console.log(`[GlobalSetup] Brand "${brandName}" not found, creating it...`);
