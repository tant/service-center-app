...
export default function EditTransferPage({ params }: EditTransferPageProps) {
  ...
  return (
    <>
      <PageHeader
        title={`Chỉnh sửa phiếu chuyển ${transfer.transfer_number || ""}`}
      />
      <div className="flex flex-1 flex-col">
        <div className="@container/main flex flex-1 flex-col gap-2">
          <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                    ...
                    {isCustomerInstalled && (
                      <div className="grid gap-2 md:col-span-2">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Hệ thống sẽ tự động cập nhật thông tin khách hàng cho
                          các sản phẩm được chuyển
                        </p>
                      </div>
                    )}
              ...
              {transfer.items?.some(
                ...
                <div className="rounded-md border border-yellow-300 bg-yellow-50 dark:bg-yellow-900/30 dark:border-yellow-700 p-4">
                  <div className="text-sm text-yellow-800 dark:text-yellow-300">
                    <strong>Lưu ý:</strong> Phiếu chuyển này đã có một số serial
                    được chọn. Nếu bạn thay đổi sản phẩm hoặc số lượng, các
                    serial đã chọn có thể bị xóa.
                  </div>
                </div>
              )}
