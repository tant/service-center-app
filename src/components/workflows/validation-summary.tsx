...
import type { ValidationIssue, WorkflowValidationResult } from "@/lib/workflow-validation";
    ...
    <div className={`space-y-2 ${className || ''}`}>
      ...
      {errors.length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertTitle>
            {errors.length === 1 ? 'Có 1 lỗi cần sửa' : `Có ${errors.length} lỗi cần sửa`}
          </AlertTitle>
          <AlertDescription>
      ...
      {/* Warnings */}
      {warnings.length > 0 && (
        <Alert variant="default" className="border-yellow-500 bg-yellow-50 text-yellow-900 dark:bg-yellow-950 dark:text-yellow-100">
          <AlertTriangle className="h-4 w-4 text-yellow-600" />
          <AlertTitle>
            {warnings.length === 1 ? 'Có 1 cảnh báo' : `Có ${warnings.length} cảnh báo`}
          </AlertTitle>
          <AlertDescription>
            <ul className="mt-2 list-disc space-y-1 pl-4">
            ...
            <p className="mt-2 text-xs opacity-75">
              Cảnh báo không chặn việc lưu, nhưng nên xem xét để cải thiện quy trình.
            </p>
          </AlertDescription>
        </Alert>
      )}
