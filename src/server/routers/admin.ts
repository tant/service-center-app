...
export const adminRouter = router({
  ...
  manual: publicProcedure
    .mutation(async ({ rawInput }) => {
  ...
  setup: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      console.log('📥 MUTATION: Input received:', JSON.stringify(input, null, 2));
      console.log('🏗️ MUTATION: Context available:', !!ctx);
      console.log('🔍 MUTATION: Input type:', typeof input);
      console.log('🔍 MUTATION: Input keys:', input ? Object.keys(input) : 'No input');

      const { password } = input;
      const { supabaseAdmin } = ctx;

      console.log('🔐 MUTATION: Password received length:', password?.length || 0);
      console.log('🗄️ MUTATION: Supabase admin client available:', !!supabaseAdmin);
      ...
      try {
        ...
        console.log('   - ADMIN_EMAIL:', !!adminEmail, adminEmail ? `(${adminEmail})` : '(missing)');
        console.log('   - ADMIN_PASSWORD:', !!adminPassword, adminPassword ? `(${adminPassword.length} chars)` : '(missing)');
        console.log('   - ADMIN_NAME:', !!adminName, adminName ? `(${adminName})` : '(missing)');
        ...
        if (!setupPassword || !adminEmail || !adminPassword || !adminName || !supabaseUrl || !supabaseAnonKey) {
          ...
          throw new TRPCError({
            ...
            message: 'Missing required environment variables for setup. Please check SETUP_PASSWORD, ADMIN_EMAIL, ADMIN_PASSWORD, and ADMIN_NAME.',
          });
        }

        console.log('🔐 MUTATION: Checking password match...');
        console.log('🔐 MUTATION: Provided password length:', password.length);
        console.log('🔐 MUTATION: Expected password length:', setupPassword.length);
        ...
        try {
          ...
          if (existingProfile) {
            console.log('❌ DATABASE: Admin user already exists with user_id:', existingProfile.user_id);
            throw new TRPCError({
              code: 'BAD_REQUEST',
          ...
          console.log('✅ DATABASE: No existing admin profile found, proceeding with setup');
        } catch (profileCheckError: any) {
          console.log('🔍 DATABASE: Profile check threw error:', profileCheckError);
          ...
          if (profileCheckError.code !== '42P01') { // 42P01 is "UndefinedTable"
            console.error('❌ DATABASE: Profile check failed with unexpected error:', profileCheckError);
            throw new TRPCError({
              code: 'INTERNAL_SERVER_ERROR',
          ...
          console.log('⚠️ DATABASE: Profiles table may not exist (42P01), continuing with setup');
        }
        ...
        const mutationDuration = Date.now() - mutationStartTime;
        console.error('❌ MUTATION: Setup failed after', mutationDuration + 'ms');
        ...
        if (error instanceof TRPCError) {
          console.log('🔄 MUTATION: Re-throwing TRPCError with code:', error.code);
          throw error;
        }
