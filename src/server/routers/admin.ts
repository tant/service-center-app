...
export const adminRouter = router({
  ...
  createRandomIssue: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const { supabaseAdmin } = ctx;
      // Find manager/admin profile ids
      ...
      const approvedById = managerProfile?.id || adminProfile?.id;
      if (!createdById || !approvedById) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Missing admin/manager profile" });

      // Choose source virtual warehouse with available stock
      let sourceVwId = input?.virtualWarehouseId || "";
      if (!sourceVwId) {
        const { data: vwWithStock } = await supabaseAdmin
