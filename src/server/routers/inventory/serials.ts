...
export const serialsRouter = router({
   ...
   * Expected format: serial_number, warranty_start_date (optional), warranty_months (optional)
   */
  bulkImportCSV: publicProcedure.use(requireManagerOrAbove)
    ...
    .mutation(async ({ ctx, input }) => {
      // Parse CSV (simple parsing, assumes comma-separated with optional header)
      ...
      for (let i = 0; i < dataLines.length; i++) {
        ...
        const serial = {
          ...
          warrantyStartDate: parts[1] || undefined,
          warrantyMonths: parts[2] ? parseInt(parts[2], 10) : undefined,
        };

        // Validate warranty months if provided
        if (serial.warrantyMonths !== undefined && isNaN(serial.warrantyMonths)) {
          errors.push(`Line ${i + 1}: Invalid warranty months: ${parts[2]}`);
          continue;
        }
  ...
  getSerialHistory: publicProcedure.use(requireManagerOrAbove)
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        warranty: {
          startDate: physicalProduct.warranty_start_date,
          endDate: physicalProduct.warranty_end_date,
          months: physicalProduct.warranty_months,
        },
