...
export const serviceRequestRouter = router({
  ...
  submit: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const normalizedEmail = input.customer_email?.trim() ?? "";
      const customerEmail =
        normalizedEmail.length > 0 ? normalizedEmail.toLowerCase() : null;
  ...
  updateDraft: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const normalizedEmail = input.data.customer_email?.trim() ?? "";
      const customerEmail =
        normalizedEmail.length > 0 ? normalizedEmail.toLowerCase() : null;
      ...
      const { error: updateError } = await ctx.supabaseAdmin
        ...
        .update({
          ...
          issue_description: input.data.issue_overview ?? null,
          receipt_status: input.data.receipt_status,
          delivery_method: input.data.delivery_method,
          delivery_address:
            input.data.delivery_method === "delivery"
              ? input.data.delivery_address ?? null
              : null,
  ...
  track: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const maskedEmail = request.customer_email
        ? request.customer_email.replace(
            /^(.{3})(.*)(@.*)$/,
      ...
      const receivedCompleted = !["draft", "submitted", "pickingup"].includes(request.status);
      const processingCompleted = ["processing", "completed"].includes(request.status);
      ...
      const timeline = [
        ...
        },
        {
          ...
          label: "Received",
          timestamp: receivedCompleted ? request.updated_at : null,
      ...
      const metadataMap = new Map<
        string,
        { brand: string | null; model: string | null; purchaseDate: string | null }
      >();

      if (serialNumbers.length > 0) {
        const { data: productRows } = await ctx.supabaseAdmin
          .from("physical_products")
          .select(
            `
            serial_number,
        ...
        (productRows ?? []).forEach((row) => {
          const productField = Array.isArray(row.product)
            ? row.product[0]
            : row.product;
          const brandField = productField?.brand
            ? Array.isArray(productField.brand)
              ? productField.brand[0]
              : productField.brand
            : null;
          const brandName =
            typeof brandField?.name === "string"
              ? brandField.name
              : typeof brandField === "string"
              ? brandField
              : null;
          const modelName =
            typeof productField?.model === "string"
              ? productField.model
              : typeof productField?.name === "string"
              ? productField.name
              : null;

          metadataMap.set(row.serial_number, {
            brand: brandName,
            model: modelName,
      ...
      const requestItems = rawItems.map((item) => {
        const ticketData = item.ticket
          ? Array.isArray(item.ticket)
            ? item.ticket[0]
            : item.ticket
          : null;
        const metadata = metadataMap.get(item.serial_number ?? "");

        return {
          id: item.id,
          product_brand: metadata?.brand ?? null,
          product_model: metadata?.model ?? null,
          serial_number: item.serial_number,
          purchase_date: metadata?.purchaseDate ?? null,
  ...
  getDetails: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (serialNumbers.length > 0) {
        const { data: productRows, error: productError } = await ctx.supabaseAdmin
          .from("physical_products")
          .select(
            `
            serial_number,
        ...
        if (!productError && Array.isArray(productRows)) {
          ...
          productRows.forEach((row) => {
            ...
            const warrantyEndDate =
              row.user_warranty_end_date ??
              row.manufacturer_warranty_end_date ??
              null;
          ...
          items.forEach((item, index) => {
            ...
            const metadata = metadataMap.get(serial);
            if (!metadata) {
              return;
            }
            items[index].product_brand = metadata.brand;
            items[index].product_model = metadata.model;
            items[index].purchase_date = metadata.purchaseDate;
            items[index].warranty_end_date = metadata.warrantyEnd;
            items[index].warranty_status = getWarrantyStatus(metadata.warrantyEnd);
            items[index].warranty_days_remaining =
              metadata.warrantyEnd !== null
                ? getRemainingDays(metadata.warrantyEnd)
                : null;
          });
        }
      }
        ...
        .update({
          ...
          delivery_address:
            input.data.delivery_method === "delivery"
              ? input.data.delivery_address ?? null
              : null,
          preferred_schedule: input.data.preferred_schedule ?? null,
          pickup_notes: input.data.pickup_notes ?? null,
          status: 'submitted',
          updated_at: new Date().toISOString(),
        })
        .eq("id", input.request_id);
