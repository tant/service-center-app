...
function createSupabaseClients() {
  ...
  console.log('   - SUPABASE_URL:', !!supabaseUrl, supabaseUrl ? `(${supabaseUrl.substring(0, 30)}...)` : '(missing)');
  console.log('   - ANON_KEY:', !!supabaseAnonKey, supabaseAnonKey ? `(${supabaseAnonKey.substring(0, 20)}...)` : '(missing)');
  console.log('   - SERVICE_ROLE_KEY:', !!supabaseServiceRoleKey, supabaseServiceRoleKey ? `(${supabaseServiceRoleKey.substring(0, 20)}...)` : '(missing)');

  if (!supabaseUrl) {
    console.error('❌ DATABASE: Missing NEXT_PUBLIC_SUPABASE_URL in environment variables');
    throw new Error('Missing NEXT_PUBLIC_SUPABASE_URL in environment variables');
  }

  if (!supabaseAnonKey) {
    console.error('❌ DATABASE: Missing NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY in environment variables');
    throw new Error('Missing NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY in environment variables');
  }

  if (!supabaseServiceRoleKey) {
    console.error('❌ DATABASE: Missing SUPABASE_SERVICE_ROLE_KEY in environment variables');
