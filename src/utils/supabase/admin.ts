...
if (!supabaseUrl) {
  console.error("❌ [ADMIN] Missing NEXT_PUBLIC_SUPABASE_URL in environment variables");
  throw new Error("Missing NEXT_PUBLIC_SUPABASE_URL in environment variables");
}

if (!supabaseServiceRoleJWT) {
  console.error("❌ [ADMIN] Missing SUPABASE_SERVICE_ROLE_JWT in environment variables");
...
console.log(`⏱️ [ADMIN] Admin client creation took ${(endTime - startTime).toFixed(2)}ms`);
...
supabaseAdmin.from = function(table: string) {
  ...
  console.log(`🔑 [ADMIN] Using admin client for table "${table}" (usage #${adminClientUsageCount})`);
  console.log(`📍 [ADMIN] Called from: ${caller} (${operationCount + 1} times for this table)`);

  if (adminClientUsageCount > 10) {
    console.warn(`⚠️ [ADMIN] HIGH USAGE: Admin client used ${adminClientUsageCount} times!`);
    console.warn(`⚠️ [ADMIN] Top operations:`, Array.from(adminOperationTracker.entries()).sort((a, b) => b[1] - a[1]).slice(0, 3));
  }
