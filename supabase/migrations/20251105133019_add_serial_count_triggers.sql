...
create type "public"."entity_type" as enum ('service_ticket', 'inventory_receipt', 'inventory_issue', 'inventory_transfer', 'service_request');

create type "public"."movement_type" as enum ('receipt', 'transfer', 'assignment', 'return', 'disposal');

create type "public"."physical_product_status" as enum ('draft', 'active', 'transferring', 'issued', 'disposed');
...
create type "public"."receipt_status" as enum ('received', 'pending_receipt');

create type "public"."request_status" as enum ('draft', 'submitted', 'pickingup', 'received', 'processing', 'completed', 'cancelled');
...
alter table "public"."email_notifications" enable row level security;
  ...
  create table "public"."entity_task_history" (
    "id" uuid not null default gen_random_uuid(),
...
alter table "public"."parts" enable row level security;
  ...
  create table "public"."physical_products" (
    ...
    "status" public.physical_product_status not null default 'draft'::public.physical_product_status,
...
alter table "public"."rma_batches" enable row level security;
  ...
  create table "public"."service_requests" (
    ...
    "receipt_status" public.receipt_status not null default 'received'::public.receipt_status,
...
alter table "public"."tasks" enable row level security;
  ...
  create table "public"."ticket_workflow_changes" (
    "id" uuid not null default gen_random_uuid(),
    "ticket_id" uuid not null,
...
CREATE UNIQUE INDEX entity_task_history_pkey ON public.entity_task_history USING btree (id);

CREATE UNIQUE INDEX entity_tasks_entity_sequence_unique ON public.entity_tasks USING btree (entity_type, entity_id, sequence_order);

CREATE UNIQUE INDEX entity_tasks_pkey ON public.entity_tasks USING btree (id);
...
CREATE INDEX idx_physical_products_previous_virtual_warehouse ON public.physical_products USING btree (previous_virtual_warehouse_id) WHERE (previous_virtual_warehouse_id IS NOT NULL);

CREATE INDEX idx_physical_products_serial_number ON public.physical_products USING btree (serial_number);

CREATE INDEX idx_physical_products_status ON public.physical_products USING btree (status);

CREATE INDEX idx_physical_products_virtual_warehouse ON public.physical_products USING btree (virtual_warehouse_id) WHERE (virtual_warehouse_id IS NOT NULL);
...
CREATE INDEX idx_service_tickets_workflow ON public.service_tickets USING btree (workflow_id) WHERE (workflow_id IS NOT NULL);
...
CREATE UNIQUE INDEX tasks_name_unique ON public.tasks USING btree (name);

CREATE UNIQUE INDEX tasks_pkey ON public.tasks USING btree (id);

CREATE UNIQUE INDEX ticket_workflow_changes_pkey ON public.ticket_workflow_changes USING btree (id);
...
CREATE UNIQUE INDEX workflow_tasks_workflow_sequence_unique ON public.workflow_tasks USING btree (workflow_id, sequence_order);

CREATE UNIQUE INDEX workflows_name_unique ON public.workflows USING btree (name);
...
alter table "public"."entity_task_history" add constraint "entity_task_history_pkey" PRIMARY KEY using index "entity_task_history_pkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_pkey" PRIMARY KEY using index "entity_tasks_pkey";
...
alter table "public"."ticket_workflow_changes" add constraint "ticket_workflow_changes_pkey" PRIMARY KEY using index "ticket_workflow_changes_pkey";
...
alter table "public"."workflow_tasks" add constraint "workflow_tasks_pkey" PRIMARY KEY using index "workflow_tasks_pkey";
...
alter table "public"."entity_task_history" add constraint "entity_task_history_changed_by_id_fkey" FOREIGN KEY (changed_by_id) REFERENCES public.profiles(id) ON DELETE SET NULL not valid;

alter table "public"."entity_task_history" validate constraint "entity_task_history_changed_by_id_fkey";

alter table "public"."entity_task_history" add constraint "entity_task_history_task_id_fkey" FOREIGN KEY (task_id) REFERENCES public.entity_tasks(id) ON DELETE CASCADE not valid;

alter table "public"."entity_task_history" validate constraint "entity_task_history_task_id_fkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_assigned_to_id_fkey" FOREIGN KEY (assigned_to_id) REFERENCES public.profiles(id) ON DELETE SET NULL not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_assigned_to_id_fkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_blocked_requires_reason" CHECK (((status <> 'blocked'::public.task_status) OR (blocked_reason IS NOT NULL))) not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_blocked_requires_reason";

alter table "public"."entity_tasks" add constraint "entity_tasks_completed_requires_notes" CHECK (((status <> 'completed'::public.task_status) OR (completion_notes IS NOT NULL))) not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_completed_requires_notes";

alter table "public"."entity_tasks" add constraint "entity_tasks_created_by_id_fkey" FOREIGN KEY (created_by_id) REFERENCES public.profiles(id) ON DELETE SET NULL not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_created_by_id_fkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_entity_sequence_unique" UNIQUE using index "entity_tasks_entity_sequence_unique";

alter table "public"."entity_tasks" add constraint "entity_tasks_sequence_positive" CHECK ((sequence_order > 0)) not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_sequence_positive";

alter table "public"."entity_tasks" add constraint "entity_tasks_started_at_before_completed" CHECK (((started_at IS NULL) OR (completed_at IS NULL) OR (started_at <= completed_at))) not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_started_at_before_completed";

alter table "public"."entity_tasks" add constraint "entity_tasks_task_id_fkey" FOREIGN KEY (task_id) REFERENCES public.tasks(id) ON DELETE RESTRICT not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_task_id_fkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_workflow_id_fkey" FOREIGN KEY (workflow_id) REFERENCES public.workflows(id) ON DELETE SET NULL not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_workflow_id_fkey";

alter table "public"."entity_tasks" add constraint "entity_tasks_workflow_task_id_fkey" FOREIGN KEY (workflow_task_id) REFERENCES public.workflow_tasks(id) ON DELETE SET NULL not valid;

alter table "public"."entity_tasks" validate constraint "entity_tasks_workflow_task_id_fkey";
...
alter table "public"."physical_products" add constraint "physical_products_previous_virtual_warehouse_id_fkey" FOREIGN KEY (previous_virtual_warehouse_id) REFERENCES public.virtual_warehouses(id) ON DELETE SET NULL not valid;

alter table "public"."physical_products" validate constraint "physical_products_previous_virtual_warehouse_id_fkey";
...
alter table "public"."physical_products" add constraint "physical_products_virtual_warehouse_id_fkey" FOREIGN KEY (virtual_warehouse_id) REFERENCES public.virtual_warehouses(id) ON DELETE RESTRICT not valid;

alter table "public"."physical_products" validate constraint "physical_products_virtual_warehouse_id_fkey";
...
alter table "public"."service_tickets" add constraint "service_tickets_workflow_id_fkey" FOREIGN KEY (workflow_id) REFERENCES public.workflows(id) ON DELETE SET NULL not valid;

alter table "public"."service_tickets" validate constraint "service_tickets_workflow_id_fkey";
...
alter table "public"."stock_issue_serials" add constraint "stock_issue_serials_physical_product_id_fkey" FOREIGN KEY (physical_product_id) REFERENCES public.physical_products(id) ON DELETE SET NULL not valid;
...
alter table "public"."stock_transfer_serials" add constraint "stock_transfer_serials_physical_product_id_fkey" FOREIGN KEY (physical_product_id) REFERENCES public.physical_products(id) ON DELETE SET NULL not valid;
...
alter table "public"."tasks" add constraint "tasks_name_unique" UNIQUE using index "tasks_name_unique";

alter table "public"."ticket_workflow_changes" add constraint "ticket_workflow_changes_changed_by_id_fkey" FOREIGN KEY (changed_by_id) REFERENCES public.profiles(id) ON DELETE RESTRICT not valid;

alter table "public"."ticket_workflow_changes" validate constraint "ticket_workflow_changes_changed_by_id_fkey";

alter table "public"."ticket_workflow_changes" add constraint "ticket_workflow_changes_new_workflow_id_fkey" FOREIGN KEY (new_workflow_id) REFERENCES public.workflows(id) ON DELETE SET NULL not valid;

alter table "public"."ticket_workflow_changes" validate constraint "ticket_workflow_changes_new_workflow_id_fkey";

alter table "public"."ticket_workflow_changes" add constraint "ticket_workflow_changes_old_workflow_id_fkey" FOREIGN KEY (old_workflow_id) REFERENCES public.workflows(id) ON DELETE SET NULL not valid;

alter table "public"."ticket_workflow_changes" validate constraint "ticket_workflow_changes_old_workflow_id_fkey";

alter table "public"."ticket_workflow_changes" add constraint "ticket_workflow_changes_ticket_id_fkey" FOREIGN KEY (ticket_id) REFERENCES public.service_tickets(id) ON DELETE CASCADE not valid;

alter table "public"."ticket_workflow_changes" validate constraint "ticket_workflow_changes_ticket_id_fkey";
...
alter table "public"."workflow_tasks" add constraint "workflow_tasks_sequence_positive" CHECK ((sequence_order > 0)) not valid;

alter table "public"."workflow_tasks" validate constraint "workflow_tasks_sequence_positive";

alter table "public"."workflow_tasks" add constraint "workflow_tasks_task_id_fkey" FOREIGN KEY (task_id) REFERENCES public.tasks(id) ON DELETE RESTRICT not valid;

alter table "public"."workflow_tasks" validate constraint "workflow_tasks_task_id_fkey";

alter table "public"."workflow_tasks" add constraint "workflow_tasks_workflow_id_fkey" FOREIGN KEY (workflow_id) REFERENCES public.workflows(id) ON DELETE CASCADE not valid;

alter table "public"."workflow_tasks" validate constraint "workflow_tasks_workflow_id_fkey";

alter table "public"."workflow_tasks" add constraint "workflow_tasks_workflow_sequence_unique" UNIQUE using index "workflow_tasks_workflow_sequence_unique";

alter table "public"."workflows" add constraint "workflows_created_by_id_fkey" FOREIGN KEY (created_by_id) REFERENCES public.profiles(id) ON DELETE RESTRICT not valid;

alter table "public"."workflows" validate constraint "workflows_created_by_id_fkey";

alter table "public"."workflows" add constraint "workflows_name_unique" UNIQUE using index "workflows_name_unique";
...
CREATE OR REPLACE FUNCTION public.get_aggregated_stock(search_term text DEFAULT NULL::text)
 RETURNS TABLE(product_id uuid, product_name text, sku text, total_declared bigint, total_actual bigint, serial_gap bigint, stock_status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
AS $function$
...
BEGIN
  ...
  -- Also insert into ticket_workflow_changes for backward compatibility
  INSERT INTO public.ticket_workflow_changes (
    ticket_id,
    old_template_id,
    new_template_id,
    reason,
...
create or replace view "public"."v_low_stock_alerts" as  SELECT p.id AS product_id,
   ...
   FROM (((public.product_stock_thresholds pst
     ...
     LEFT JOIN ( SELECT pp.product_id,
          ...
          GROUP BY pp.product_id, vw.warehouse_type) stock ON (((stock.product_id = p.id) AND (stock.warehouse_type = pst.warehouse_type))))
...
create or replace view "public"."v_stock_summary" as  SELECT p.id AS product_id,
    p.name AS product_name,
    p.sku,
    ...
    COALESCE(( SELECT (count(*))::integer AS count
           FROM public.physical_products pp
          WHERE ((pp.product_id = p.id) AND (pp.virtual_warehouse_id = vw.id))), 0) AS actual_serial_count,
    (pws.declared_quantity - COALESCE(( SELECT (count(*))::integer AS count
           FROM public.physical_products pp
          WHERE ((pp.product_id = p.id) AND (pp.virtual_warehouse_id = vw.id))), 0)) AS serial_gap,
    pws.initial_stock_entry,
    pst.minimum_quantity,
    pst.reorder_quantity,
        CASE
            WHEN (pws.declared_quantity = 0) THEN 'critical'::text
            WHEN ((pst.minimum_quantity IS NOT NULL) AND (pws.declared_quantity <= pst.minimum_quantity)) THEN 'warning'::text
            ELSE 'ok'::text
        END AS stock_status,
    pws.created_at,
    pws.updated_at
   FROM ((((public.product_warehouse_stock pws
     ...
     LEFT JOIN public.product_stock_thresholds pst ON (((pst.product_id = pws.product_id) AND (pst.warehouse_type = vw.warehouse_type))));
...
create or replace view "public"."v_task_progress_summary" as  SELECT st.id AS ticket_id,
    ...
    count(*) FILTER (WHERE (et.status = 'pending'::public.task_status)) AS pending_tasks,
    count(*) FILTER (WHERE (et.status = 'in_progress'::public.task_status)) AS in_progress_tasks,
    count(*) FILTER (WHERE (et.status = 'completed'::public.task_status)) AS completed_tasks,
    count(*) FILTER (WHERE (et.status = 'blocked'::public.task_status)) AS blocked_tasks,
    count(*) FILTER (WHERE (et.status = 'skipped'::public.task_status)) AS skipped_tasks,
    count(*) FILTER (WHERE (et.is_required = true)) AS required_tasks,
    count(*) FILTER (WHERE ((et.is_required = true) AND (et.status = 'completed'::public.task_status))) AS required_completed,
    round((((count(*) FILTER (WHERE (et.status = 'completed'::public.task_status)))::numeric / (NULLIF(count(*), 0))::numeric) * (100)::numeric), 2) AS completion_percentage,
    round((((count(*) FILTER (WHERE ((et.is_required = true) AND (et.status = 'completed'::public.task_status))))::numeric / (NULLIF(count(*) FILTER (WHERE (et.is_required = true)), 0))::numeric) * (100)::numeric), 2) AS required_completion_percentage,
    min(et.started_at) AS first_task_started_at,
    max(et.completed_at) AS last_task_completed_at,
    sum(
        CASE
            WHEN ((et.completed_at IS NOT NULL) AND (et.started_at IS NOT NULL)) THEN (EXTRACT(epoch FROM (et.completed_at - et.started_at)) / (60)::numeric)
            ELSE (0)::numeric
        END) AS total_minutes_spent,
    ( SELECT jsonb_build_object('id', et2.id, 'name', et2.name, 'sequence_order', et2.sequence_order, 'assigned_to_id', et2.assigned_to_id) AS jsonb_build_object
           FROM public.entity_tasks et2
          WHERE ((et2.entity_type = 'service_ticket'::public.entity_type) AND (et2.entity_id = st.id) AND (et2.status = 'pending'::public.task_status))
          ORDER BY et2.sequence_order
         LIMIT 1) AS next_pending_task,
    ( SELECT jsonb_agg(jsonb_build_object('id', et3.id, 'name', et3.name, 'sequence_order', et3.sequence_order, 'blocked_reason', et3.blocked_reason) ORDER BY et3.sequence_order) AS jsonb_agg
           FROM public.entity_tasks et3
          WHERE ((et3.entity_type = 'service_ticket'::public.entity_type) AND (et3.entity_id = st.id) AND (et3.status = 'blocked'::public.task_status))) AS blocked_tasks_detail,
    st.created_at AS ticket_created_at,
    st.updated_at AS ticket_updated_at
   FROM ((public.service_tickets st
     LEFT JOIN public.workflows wf ON ((st.workflow_id = wf.id)))
     LEFT JOIN public.entity_tasks et ON (((et.entity_type = 'service_ticket'::public.entity_type) AND (et.entity_id = st.id))))
  GROUP BY st.id, st.ticket_number, st.status, wf.id, wf.name, wf.strict_sequence, st.created_at, st.updated_at
  ORDER BY st.created_at DESC;


create or replace view "public"."v_task_statistics" as  SELECT t.id AS task_id,
    t.name AS task_name,
    t.entity_type,
    t.entity_id,
    t.status,
    ...
    t.estimated_duration_minutes,
        CASE
            WHEN ((t.completed_at IS NOT NULL) AND (t.due_date IS NOT NULL)) THEN (t.completed_at > t.due_date)
            ELSE NULL::boolean
        END AS is_overdue,
        CASE
            WHEN (t.completed_at > t.due_date) THEN (EXTRACT(epoch FROM (t.completed_at - t.due_date)) / (60)::numeric)
            ELSE (0)::numeric
        END AS overdue_minutes
   FROM (public.entity_tasks t
     LEFT JOIN public.profiles p ON ((t.assigned_to_id = p.id)));


create or replace view "public"."v_warehouse_stock_levels" as  SELECT vw.id AS virtual_warehouse_id,
    vw.name AS virtual_warehouse_name,
    vw.warehouse_type,
    ...
    count(pp.id) FILTER (WHERE (((pp.user_warranty_end_date IS NOT NULL) AND (pp.user_warranty_end_date > (CURRENT_DATE + '30 days'::interval))) OR ((pp.user_warranty_end_date IS NULL) AND (pp.manufacturer_warranty_end_date IS NOT NULL) AND (pp.manufacturer_warranty_end_date > (CURRENT_DATE + '30 days'::interval))))) AS active_warranty_count,
    count(pp.id) FILTER (WHERE (((pp.user_warranty_end_date IS NOT NULL) AND (pp.user_warranty_end_date > CURRENT_DATE) AND (pp.user_warranty_end_date <= (CURRENT_DATE + '30 days'::interval))) OR ((pp.user_warranty_end_date IS NULL) AND (pp.manufacturer_warranty_end_date IS NOT NULL) AND (pp.manufacturer_warranty_end_date > CURRENT_DATE) AND (pp.manufacturer_warranty_end_date <= (CURRENT_DATE + '30 days'::interval))))) AS expiring_soon_count,
    count(pp.id) FILTER (WHERE (((pp.user_warranty_end_date IS NOT NULL) AND (pp.user_warranty_end_date <= CURRENT_DATE)) OR ((pp.user_warranty_end_date IS NULL) AND (pp.manufacturer_warranty_end_date IS NOT NULL) AND (pp.manufacturer_warranty_end_date <= CURRENT_DATE)))) AS expired_count,
    ...
    pst.alert_enabled,
        CASE
            WHEN ((pst.minimum_quantity IS NOT NULL) AND (count(pp.id) < pst.minimum_quantity)) THEN true
            ELSE false
...
create or replace view "public"."v_warranty_expiring_soon" as  SELECT pp.id AS physical_product_id,
    ...
    GREATEST(pp.manufacturer_warranty_end_date, pp.user_warranty_end_date) AS effective_warranty_end_date,
        CASE
            WHEN (GREATEST(pp.manufacturer_warranty_end_date, pp.user_warranty_end_date) < CURRENT_DATE) THEN 'expired'::text
            WHEN (GREATEST(pp.manufacturer_warranty_end_date, pp.user_warranty_end_date) <= (CURRENT_DATE + '30 days'::interval)) THEN 'expiring_soon'::text
            ELSE 'active'::text
  ...
  WHERE (((pp.manufacturer_warranty_end_date IS NOT NULL) OR (pp.user_warranty_end_date IS NOT NULL)) AND (GREATEST(pp.manufacturer_warranty_end_date, pp.user_warranty_end_date) <= (CURRENT_DATE + '90 days'::interval)))
  ORDER BY GREATEST(pp.manufacturer_warranty_end_date, pp.user_warranty_end_date), b.name, p.name;
...
grant delete on table "public"."entity_task_history" to "anon";

grant insert on table "public"."entity_task_history" to "anon";

grant references on table "public"."entity_task_history" to "anon";

grant select on table "public"."entity_task_history" to "anon";

grant trigger on table "public"."entity_task_history" to "anon";

grant truncate on table "public"."entity_task_history" to "anon";

grant update on table "public"."entity_task_history" to "anon";

grant delete on table "public"."entity_task_history" to "authenticated";

grant insert on table "public"."entity_task_history" to "authenticated";

grant references on table "public"."entity_task_history" to "authenticated";

grant select on table "public"."entity_task_history" to "authenticated";

grant trigger on table "public"."entity_task_history" to "authenticated";

grant truncate on table "public"."entity_task_history" to "authenticated";

grant update on table "public"."entity_task_history" to "authenticated";

grant delete on table "public"."entity_task_history" to "service_role";

grant insert on table "public"."entity_task_history" to "service_role";

grant references on table "public"."entity_task_history" to "service_role";

grant select on table "public"."entity_task_history" to "service_role";

grant trigger on table "public"."entity_task_history" to "service_role";

grant truncate on table "public"."entity_task_history" to "service_role";

grant update on table "public"."entity_task_history" to "service_role";

grant delete on table "public"."entity_tasks" to "anon";

grant insert on table "public"."entity_tasks" to "anon";

grant references on table "public"."entity_tasks" to "anon";

grant select on table "public"."entity_tasks" to "anon";

grant trigger on table "public"."entity_tasks" to "anon";

grant truncate on table "public"."entity_tasks" to "anon";

grant update on table "public"."entity_tasks" to "anon";

grant delete on table "public"."entity_tasks" to "authenticated";

grant insert on table "public"."entity_tasks" to "authenticated";

grant references on table "public"."entity_tasks" to "authenticated";

grant select on table "public"."entity_tasks" to "authenticated";

grant trigger on table "public"."entity_tasks" to "authenticated";

grant truncate on table "public"."entity_tasks" to "authenticated";

grant update on table "public"."entity_tasks" to "authenticated";

grant delete on table "public"."entity_tasks" to "service_role";

grant insert on table "public"."entity_tasks" to "service_role";

grant references on table "public"."entity_tasks" to "service_role";

grant select on table "public"."entity_tasks" to "service_role";

grant trigger on table "public"."entity_tasks" to "service_role";

grant truncate on table "public"."entity_tasks" to "service_role";

grant update on table "public"."entity_tasks" to "service_role";
...
grant delete on table "public"."ticket_workflow_changes" to "anon";

grant insert on table "public"."ticket_workflow_changes" to "anon";

grant references on table "public"."ticket_workflow_changes" to "anon";

grant select on table "public"."ticket_workflow_changes" to "anon";

grant trigger on table "public"."ticket_workflow_changes" to "anon";

grant truncate on table "public"."ticket_workflow_changes" to "anon";

grant update on table "public"."ticket_workflow_changes" to "anon";

grant delete on table "public"."ticket_workflow_changes" to "authenticated";

grant insert on table "public"."ticket_workflow_changes" to "authenticated";

grant references on table "public"."ticket_workflow_changes" to "authenticated";

grant select on table "public"."ticket_workflow_changes" to "authenticated";

grant trigger on table "public"."ticket_workflow_changes" to "authenticated";

grant truncate on table "public"."ticket_workflow_changes" to "authenticated";

grant update on table "public"."ticket_workflow_changes" to "authenticated";

grant delete on table "public"."ticket_workflow_changes" to "service_role";

grant insert on table "public"."ticket_workflow_changes" to "service_role";

grant references on table "public"."ticket_workflow_changes" to "service_role";

grant select on table "public"."ticket_workflow_changes" to "service_role";

grant trigger on table "public"."ticket_workflow_changes" to "service_role";

grant truncate on table "public"."ticket_workflow_changes" to "service_role";

grant update on table "public"."ticket_workflow_changes" to "service_role";
...
grant delete on table "public"."workflow_tasks" to "anon";

grant insert on table "public"."workflow_tasks" to "anon";

grant references on table "public"."workflow_tasks" to "anon";

grant select on table "public"."workflow_tasks" to "anon";

grant trigger on table "public"."workflow_tasks" to "anon";

grant truncate on table "public"."workflow_tasks" to "anon";

grant update on table "public"."workflow_tasks" to "anon";

grant delete on table "public"."workflow_tasks" to "authenticated";

grant insert on table "public"."workflow_tasks" to "authenticated";

grant references on table "public"."workflow_tasks" to "authenticated";

grant select on table "public"."workflow_tasks" to "authenticated";

grant trigger on table "public"."workflow_tasks" to "authenticated";

grant truncate on table "public"."workflow_tasks" to "authenticated";

grant update on table "public"."workflow_tasks" to "authenticated";

grant delete on table "public"."workflow_tasks" to "service_role";

grant insert on table "public"."workflow_tasks" to "service_role";

grant references on table "public"."workflow_tasks" to "service_role";

grant select on table "public"."workflow_tasks" to "service_role";

grant trigger on table "public"."workflow_tasks" to "service_role";

grant truncate on table "public"."workflow_tasks" to "service_role";

grant update on table "public"."workflow_tasks" to "service_role";
...
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]) OR (public.is_technician() AND (id IN ( SELECT st.customer_id
   FROM (public.service_tickets st
     JOIN public.entity_tasks et ON (((et.entity_type = 'service_ticket'::public.entity_type) AND (et.entity_id = st.id))))
  WHERE (et.assigned_to_id = ( SELECT auth.uid() AS uid)))))));
...
with check (public.has_any_role(ARRAY['admin'::text, 'manager'::text]));
  ...
  create policy "entity_task_history_authenticated_read"
...
using (true);
  ...
  create policy "entity_task_history_system_insert"
  on "public"."entity_task_history"
  as permissive
  for insert
  to authenticated
...
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text]) OR ((EXISTS ( SELECT 1
  ...
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid)) AND (entity_tasks.entity_id = physical_products.current_ticket_id)))) AND public.is_technician())))
...
using (public.is_admin_or_manager());
  ...
  for insert
  to authenticated
with check ((public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]) OR (public.is_technician() AND (ticket_id IN ( SELECT entity_tasks.entity_id
   FROM public.entity_tasks
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid))))))));
  ...
  for select
  to authenticated
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]) OR (public.is_technician() AND (ticket_id IN ( SELECT entity_tasks.entity_id
   FROM public.entity_tasks
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid))))))));
...
using (public.is_admin_or_manager());
  ...
  for insert
  to authenticated
with check ((public.has_any_role(ARRAY['admin'::text, 'manager'::text]) OR (public.is_technician() AND (ticket_id IN ( SELECT entity_tasks.entity_id
   FROM public.entity_tasks
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid))))))));
...
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]) OR (public.is_technician() AND (ticket_id IN ( SELECT entity_tasks.entity_id
  ...
  for update
  to authenticated
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text]) OR (public.is_technician() AND (ticket_id IN ( SELECT entity_tasks.entity_id
   FROM public.entity_tasks
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid))))))));
...
with check (public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]));
  ...
  for select
  to authenticated
using ((public.has_any_role(ARRAY['admin'::text, 'manager'::text, 'reception'::text]) OR (public.is_technician() AND (id IN ( SELECT entity_tasks.entity_id
   FROM public.entity_tasks
  WHERE ((entity_tasks.entity_type = 'service_ticket'::public.entity_type) AND (entity_tasks.assigned_to_id = ( SELECT auth.uid() AS uid))))))));
...
with check (public.has_any_role(ARRAY['admin'::text, 'manager'::text]));
  ...
  create policy "ticket_workflow_changes_admin_insert"
  on "public"."ticket_workflow_changes"
  as permissive
  for insert
  to authenticated
with check (((changed_by_id = ( SELECT auth.uid() AS uid)) AND public.has_any_role(ARRAY['admin'::text, 'manager'::text])));
  ...
  create policy "ticket_workflow_changes_authenticated_read"
  on "public"."ticket_workflow_changes"
  as permissive
  for select
  to authenticated
...
with check (public.has_any_role(ARRAY['admin'::text, 'manager'::text]));
  ...
  create policy "workflow_tasks_delete_policy"
...
CREATE TRIGGER trigger_mark_physical_product_as_transferring_on_issue AFTER INSERT ON public.stock_issue_serials FOR EACH ROW EXECUTE FUNCTION public.mark_physical_product_as_transferring_on_issue();

CREATE TRIGGER trigger_restore_active_status_on_issue_serial_removal BEFORE DELETE ON public.stock_issue_serials FOR EACH ROW EXECUTE FUNCTION public.restore_active_status_on_issue_serial_removal();

CREATE TRIGGER trigger_validate_issue_serial_physical_product BEFORE INSERT ON public.stock_issue_serials FOR EACH ROW EXECUTE FUNCTION public.validate_issue_serial_physical_product();
...
CREATE TRIGGER trigger_mark_physical_products_as_issued_on_issue_approval AFTER UPDATE ON public.stock_issues FOR EACH ROW WHEN (((new.status = 'approved'::public.stock_document_status) AND (old.status <> 'approved'::public.stock_document_status))) EXECUTE FUNCTION public.mark_physical_products_as_issued_on_issue_approval();

CREATE TRIGGER trigger_restore_active_status_on_issue_cancel AFTER UPDATE ON public.stock_issues FOR EACH ROW WHEN (((new.status = 'cancelled'::public.stock_document_status) AND (old.status <> 'cancelled'::public.stock_document_status))) EXECUTE FUNCTION public.restore_active_status_on_issue_cancel();
...
CREATE TRIGGER trigger_cleanup_draft_physical_product_on_serial_removal BEFORE DELETE ON public.stock_receipt_serials FOR EACH ROW EXECUTE FUNCTION public.cleanup_draft_physical_product_on_serial_removal();
...
CREATE TRIGGER trigger_update_receipt_serial_count_on_delete AFTER DELETE ON public.stock_receipt_serials FOR EACH ROW EXECUTE FUNCTION public.update_receipt_item_serial_count();

CREATE TRIGGER trigger_update_receipt_serial_count_on_insert AFTER INSERT ON public.stock_receipt_serials FOR EACH ROW EXECUTE FUNCTION public.update_receipt_item_serial_count();

CREATE TRIGGER trigger_activate_physical_products_on_receipt_approval AFTER UPDATE ON public.stock_receipts FOR EACH ROW WHEN (((new.status = 'approved'::public.stock_document_status) AND (old.status <> 'approved'::public.stock_document_status))) EXECUTE FUNCTION public.activate_physical_products_on_receipt_approval();

CREATE TRIGGER trigger_cleanup_draft_physical_products_on_receipt_cancel AFTER UPDATE ON public.stock_receipts FOR EACH ROW WHEN (((new.status = 'cancelled'::public.stock_document_status) AND (old.status <> 'cancelled'::public.stock_document_status))) EXECUTE FUNCTION public.cleanup_draft_physical_products_on_receipt_cancel();
...
CREATE TRIGGER trigger_mark_physical_product_as_transferring_on_transfer AFTER INSERT ON public.stock_transfer_serials FOR EACH ROW EXECUTE FUNCTION public.mark_physical_product_as_transferring_on_transfer();

CREATE TRIGGER trigger_restore_active_status_on_transfer_serial_removal BEFORE DELETE ON public.stock_transfer_serials FOR EACH ROW EXECUTE FUNCTION public.restore_active_status_on_transfer_serial_removal();
...
CREATE TRIGGER trigger_validate_transfer_serial_physical_product BEFORE INSERT ON public.stock_transfer_serials FOR EACH ROW EXECUTE FUNCTION public.validate_transfer_serial_physical_product();
...
CREATE TRIGGER trigger_restore_active_status_on_transfer_approval AFTER UPDATE ON public.stock_transfers FOR EACH ROW WHEN (((new.status = 'approved'::public.transfer_status) AND (old.status <> 'approved'::public.transfer_status))) EXECUTE FUNCTION public.restore_active_status_on_transfer_approval();

CREATE TRIGGER trigger_restore_active_status_on_transfer_cancel AFTER UPDATE ON public.stock_transfers FOR EACH ROW WHEN (((new.status = 'cancelled'::public.transfer_status) AND (old.status <> 'cancelled'::public.transfer_status))) EXECUTE FUNCTION public.restore_active_status_on_transfer_cancel();
...
CREATE TRIGGER trigger_update_stock_on_transfer_approval AFTER UPDATE ON public.stock_transfers FOR EACH ROW WHEN (((new.status = 'approved'::public.transfer_status) AND (old.status <> 'approved'::public.transfer_status))) EXECUTE FUNCTION public.update_stock_on_transfer_approval();
