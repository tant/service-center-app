-- =====================================================
-- Migration: Recreate Views with New Schema
-- Description: Update views to use virtual_warehouse_id
-- Date: 2025-10-28
-- =====================================================

-- =====================================================
-- VIEW: Stock Summary with Gap Tracking (UPDATED)
-- =====================================================
  ...
  COALESCE(
    (SELECT COUNT(*)
     ...
     WHERE pp.product_id = p.id
       AND pp.virtual_warehouse_type = vw.warehouse_type
       AND (pp.physical_warehouse_id = vw.physical_warehouse_id
            OR (pp.physical_warehouse_id IS NULL AND vw.physical_warehouse_id IS NULL))
    ), 0
...
COMMENT ON VIEW v_stock_summary IS
  'Complete stock overview with declared vs actual tracking (REDESIGNED)';

-- =====================================================
-- VIEW: Pending Approvals Dashboard (unchanged)
-- =====================================================
...
-- =====================================================
-- HELPER FUNCTIONS for tRPC (unchanged)
-- =====================================================
...
BEGIN
  ...
  FROM products p
  LEFT JOIN product_warehouse_stock pws ON pws.product_id = p.id
  LEFT JOIN virtual_warehouses vw ON vw.id = pws.virtual_warehouse_id
  LEFT JOIN product_stock_thresholds pst ON pst.product_id = p.id AND pst.warehouse_type = vw.warehouse_type;
END;
$$ LANGUAGE plpgsql;

SELECT 'Views recreated with updated schema' as status;
