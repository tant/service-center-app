...
-- Task Progress Summary View (Updated for Polymorphic Tasks)
CREATE OR REPLACE VIEW public.v_task_progress_summary AS
SELECT st.id AS ticket_id, st.ticket_number, st.status AS ticket_status, wf.id AS workflow_id, wf.name AS workflow_name, wf.strict_sequence, COUNT(*) AS total_tasks, COUNT(*) FILTER (WHERE et.status = 'pending') AS pending_tasks, COUNT(*) FILTER (WHERE et.status = 'in_progress') AS in_progress_tasks, COUNT(*) FILTER (WHERE et.status = 'completed') AS completed_tasks, COUNT(*) FILTER (WHERE et.status = 'blocked') AS blocked_tasks, COUNT(*) FILTER (WHERE et.status = 'skipped') AS skipped_tasks, COUNT(*) FILTER (WHERE et.is_required = true) AS required_tasks, COUNT(*) FILTER (WHERE et.is_required = true AND et.status = 'completed') AS required_completed, ROUND((COUNT(*) FILTER (WHERE et.status = 'completed')::NUMERIC / NULLIF(COUNT(*), 0)) * 100, 2) AS completion_percentage, ROUND((COUNT(*) FILTER (WHERE et.is_required = true AND et.status = 'completed')::NUMERIC / NULLIF(COUNT(*) FILTER (WHERE et.is_required = true), 0)) * 100, 2) AS required_completion_percentage, MIN(et.started_at) AS first_task_started_at, MAX(et.completed_at) AS last_task_completed_at, SUM(CASE WHEN et.completed_at IS NOT NULL AND et.started_at IS NOT NULL THEN EXTRACT(EPOCH FROM (et.completed_at - et.started_at)) / 60 ELSE 0 END) AS total_minutes_spent, (SELECT jsonb_build_object('id', et2.id, 'name', et2.name, 'sequence_order', et2.sequence_order, 'assigned_to_id', et2.assigned_to_id) FROM public.entity_tasks et2 WHERE et2.entity_type = 'service_ticket' AND et2.entity_id = st.id AND et2.status = 'pending' ORDER BY et2.sequence_order LIMIT 1) AS next_pending_task, (SELECT jsonb_agg(jsonb_build_object('id', et3.id, 'name', et3.name, 'sequence_order', et3.sequence_order, 'blocked_reason', et3.blocked_reason) ORDER BY et3.sequence_order) FROM public.entity_tasks et3 WHERE et3.entity_type = 'service_ticket' AND et3.entity_id = st.id AND et3.status = 'blocked') AS blocked_tasks_detail, st.created_at AS ticket_created_at, st.updated_at AS ticket_updated_at
FROM public.service_tickets st LEFT JOIN public.workflows wf ON st.workflow_id = wf.id LEFT JOIN public.entity_tasks et ON et.entity_type = 'service_ticket' AND et.entity_id = st.id
GROUP BY st.id, st.ticket_number, st.status, wf.id, wf.name, wf.strict_sequence, st.created_at, st.updated_at
ORDER BY st.created_at DESC;
COMMENT ON VIEW public.v_task_progress_summary IS 'Task completion progress summary per service ticket with next/blocked task details (using polymorphic entity_tasks)';
